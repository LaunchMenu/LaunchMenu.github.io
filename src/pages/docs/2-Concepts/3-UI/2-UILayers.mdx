...
LaunchMenu already contains 3 different types of build in transition components:
...
This choice may be worth reconsidering, but so far it hasn't been problematic in my experience.
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerPath
...
-   Model completion: If you provide an [`IMenu`]() -, [`ITextField`]() -, or [`IContent`]() data model, standard key handlers and views are automatically added if absent to make your data interactive and visualize it
-   Searchable menu: If you provide an [`IMenu`]() data model, it will automatically add a searchfield to search in that menu unless `searchable` is explicitly set to false
-   Menu content: If you provide an [`IMenu`]() data model, it will automatically show the content of the selected menu item, unless `hideItemContent` is explicitly set to true
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerModelCompletion
export default declare({
    info,
    settings,
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerSearching
export default declare({
    info,
    settings,
    async open({context, onClose}) {
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerMenuContent
const items = [
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerOverlays spoiler
export default declare({
    info,
    settings,
    async open({context, onClose}) {
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerKeyBarrier spoiler
export default declare({
    info,
    settings,
    async open({context, onClose}) {
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerCustomLayer spoiler
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/UILayerReuseLayer
...
```ts title=UILayerExtendingExample.tsx
...
LaunchMenu has several build in layers that are used throughout the application, and may also be useable as sub-layers:
...
The [`InputLayer`](https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/input/InputLayer.tsx) class takes in a [model-react field](https://tarvk.github.io/model-react/examples/build/#Field) and a config. When opened, it allows the user to update this field's value using a text field.

Below is the interface of the class:

```ts title=InputLayer.ts source=https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/input/InputLayer.tsx spoiler
...
The [`SelectLayer`](https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/select/SelectLayer.tsx) class takes in a [model-react field](https://tarvk.github.io/model-react/examples/build/#Field) and a config. When opened, it allows the user to update this field's value by selecting one of the options. Depending on the configuration it also allows for entering a custom option.

Below is the interface of the class:

```ts title=SelectLayer.ts source=https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/select/SelectLayer.tsx spoiler
...
The [`MultiSelectLayer`](https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/multiSelect/MultiSelectLayer.tsx) class takes in a [model-react field](https://tarvk.github.io/model-react/examples/build/#Field) and a config. When opened, it allows the user to update this field's value by selecting one or more options. Depending on the configuration it also allows for entering a custom option.

Below is the interface of the class:

```ts title=MultiSelectLayer.ts source=https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/multiSelect/MultiSelectLayer.tsx spoiler
...
The [ContextMenuLayer](https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/context/ContextMenuLayer.tsx) is a rather specific layer, just for correctly showing a context menu. It probably won't see much reuse within LaunchMenu for other purpose than showing the context menu, but it could serve as a simple example.

### MenuSearchLayer

The [MenuSearch](https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/menuSearch/MenuSearch.tsx) layer is a layer that takes care of all searching needs. It will show a search field, and open up a dedicated search menu when a query is entered. The constructor takes a simple configuration:

```ts title=MenuSearch.tsx source=https://github.com/LaunchMenu/LaunchMenu/blob/feature/globalKeyListener/packages/core/src/uiLayers/types/menuSearch/MenuSearch.tsx spoiler
