...
```tsx title=IMenu.tsx source=https://github.com/LaunchMenu/LaunchMenu/blob/master/packages/core/src/menus/menu/_types/IMenu.ts spoiler
export type IMenu = {
    /**
...
Items in menus can be categorized (I.e. grouped according to a category) based on the categories they specify using the [`getCategoryAction`]() <!-- TODO: add reference -->. These categories are simply menu items with a bit of extra fixed contextual data:
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuCategories
...
The example below shows off a very basic interactive category, but the possible config data is roughly equivalent to what can be passed to [`createStandardMenuItem`]() <!-- TODO: add docs reference -->.

```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuAdvancedCategories
...
There is only 1 standard view for menus as of right now: `MenuView`. This is simply a react component that can be instantiated given a menu to show. When using the [`UILayer` class]() <!-- TODO: add reference --> this view will automatically be created for any specified menus without views, but it's also possible to create your own views.
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuViews spoiler
const subitems = [
    createStandardMenuItem({
        name: "Potaters",
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuSearchHighlight
...
most of the interaction with the menu is performed through keyboard controls. We have one primary factory called `createStandardMenuKeyHandler` which allows you to create a standard keyboard controller for any menu. When using the [`UILayer` class]() <!-- TODO: add reference --> this controller will automatically be created for any specified menus without views, but it's also possible to create your own controller or customize this standard controller:

```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuKeyHandler
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuMenu
export default declare({
    info,
    settings,
    open({context, onClose}) {
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuProxiedMenu
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuPrioritizedMenu
const items = [
    {
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuProxiedPrioritizedMenu
...
`SearchMenu` allows us to easily search within a list of items using a given query. By using the [`SearchExecuter`]() <!-- TODO: add reference --> searches will automatically change when the query is updated.
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/menuSearchMenu
const items = [
    createStandardMenuItem({
        name: "Hello world",
        onExecute: () => alert("Hello!"),
...
When you open `example` you will see that only `Hello world` was found, and that `Hell` was highlighted. Since we didn't indicate `searchable: false` on the UILayer, we can still search within this menu too. You can learn more about this on the [`UILayer` page ]() <!-- TODO: add reference to searchable section -->.
