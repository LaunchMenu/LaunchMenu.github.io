...
Using the [`UILayer`]() and [`IOContext`]() components a stack of these listeners will be created. Events are then dispatched from top to bottom. Whenever propagation is stopped by the callback returning `true`, it will prevent the listeners below from receiving the event.

<!-- TODO: add references -->

The [`createStandardMenuKeyHandler`]() <!-- TODO: add reference to menu controller section --> will also use the [`keyHandlerAction`]() <!-- TODO: add reference to action page keyhandler section --> to propagate key events to items in the menu, as well as the [`contextMenuAction`]() <!-- TODO: add reference to action page keyhandler section --> to propagate events to items in the context menu of the current selection. Bindings of the `keyHandlerAction` are event listeners similar to `IKeyEventListener`, except they can indicate to stop `immediate propagation` as well, and contain some more context data:
...
For usage examples see the [`keyHandlerAction`]() <!-- TODO: add reference --> on the actions page.
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/keyHandlerEvents
export default declare({
    info,
    settings,
    open({context, onClose}) {
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/keyHandlerPatterns
...
The global instance of this class within the instance of LaunchMenu could be used to emit your own fake/virtual key events, adding global listeners, or checking if a key is currently held down. To access this instance, see the [LaunchMenu class page](). <!-- TODO: add reference -->
...
If you want to setup global shortcuts, there is no need to use this class however. A more convenient `createGlobalKeyPatternSetting` exists which can be used for creating a user alterable pattern, and has a built-in function to register callbacks for when the event triggers:
