...
This page contains an overview of common menu item actions in LaunchMenu. To learn what `actions` are exactly, you can check the general [actions page](). <!-- TODO: add reference -->
...
These executables can optionally return [commands]() <!--TODO: add reference -->. These commands will automatically be executed and are undoable by the user. If multiple items are selected, and multiple commands are obtained, all these commands are combined. This means that the user can undo and redo them all at once.
...
The `onExecute` property of the [`createStandardMenuItem`]() <!-- TODO: add reference --> creates a direct binding to this action from the passed function. You can however still choose to create your own bindings if you're creating a custom item type, or if you want to add multiple bindings on one item:
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsCustomExecuteHandler
...
The execute action also has a context-menu item. We are able to override this item for our custom handler however. This is discussed in more detail in the [... section]() <!-- TODO: add reference + text -->, but the code below is a quick example:

```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsCustomExecuteItem
...
The `exitLMExecuteHandler` is quite a peculiar handler, but can be useful in a couple of scenarios. When it's executed it essentially closes LaunchMenu, performs some callback action, and then possibly open LM up again. This behavior is for instance used by the [`copyExitPasteHandler`]() <!-- TODO: add reference --> which is used to paste some text within the program you were using before LaunchMenu.
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsSinglePrompt
...
There are a bunch of different prompt execute handlers for different kinds of data inputs. These correspond to the different available [setting types]() <!-- TODO: add reference -->.
...
In most cases items won't create bindings for this context-menu action directly, since that would result in separate context-items representing the same actions. It's still possible to do this for 1-off bindings however, as we've seen in the [`GlobalContextMenuBindings` section of the applet format page]() <!-- TODO: add reference -->. In most situations you will however want to make your action be a handler of the context-menu action:

```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsContextMenu
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsContextMenuShorthand
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsContextMenuOverride
const alertAllAction = createContextAction({
    name: "Alert all",
    contextItem: {
        priority: [Priority.MEDIUM, Priority.HIGH],
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsContextMenuFolders
...
The extraction of these categories and adding them to the menu is done by the individual [`IMenu` data models]() <!-- TODO: add reference -->.
You can learn more about categories on the [menu page]() <!-- TODO: add reference -->.

The [`createStandardMenuItem` factory]() <!-- TODO: add reference--> has a standard field for specifying categories, but below is an example where we manually specify the binding:

```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsCategory
...
The `getContentAction` is used by [`UILayers`]() <!-- TODO: add reference--> to show the content of the currently selected item. The [`createStandardMenuItem` factory]() <!-- TODO: add reference--> has a standard field for specifying a content element, which creates a binding to the `scrollableContentHandler`. The `scrollableContentHandler` is a handler for `getContentAction` which generates content that can be scrolled by the user using the keyboard, as described on the [content page]() <!-- TODO: add reference -->.

Below is an example that demonstrates how you can manually create these content bindings:

```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsContent
const items = [
    createStandardMenuItem({
        name: "Hello world",
...
This example will log the typed keys in the console whenever this item is visible and captures these events.
...
We also have a `forwardKeyEventHandler` that can be used to forward key events to other menu items. This is used by the [`createFolderMenuItem`]() <!-- TODO: add reference --> in order to forward key events to sub-menus.
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsShortcuts
...
You can learn more about key handlers on the [key handlers page]() <!-- TODO: add reference -->.
...
This action is used by the individual [`IMenu` data models]() <!-- TODO: add reference --> to call the selection listeners of items whenever appropriate.
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsListenerSelect
...
This action is used by the individual [`IMenu` data models]() <!-- TODO: add reference --> to call the cursor listeners of items whenever appropriate.
...
This action is used by the individual [`IMenu` data models]() <!-- TODO: add reference --> to call the menu change listeners of items whenever these are added or removed from the menu.
...
The `searchAction` can be used to provide `ISearchables`. This is used to obtain a collection of items that satisfy a search according to a given menu item. You can learn more about the search system as well as little bit about the search action on the [search system page]() <!-- TODO: add reference -->.
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsIdentityAugment
const baseItem = [
    createStandardMenuItem({
        name: "Hello world",
        onExecute: () => alert("Hello"),
...
```tsx title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsIdentityUsage
...
```ts title=src/index.tsx source=https://github.com/LaunchMenu/LM-applet-examples/tree/main/examples/commonActionsCopy
const items = [
    createStandardMenuItem({
