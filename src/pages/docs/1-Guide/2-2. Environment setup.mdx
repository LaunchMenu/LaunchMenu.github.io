...
<!-- TODO: update the version numbers once LM is published -->

Below is a config example with LaunchMenu dependencies added:

```json title=package.json highlight=9:21
{
    "name": "applet-hello-world",
    "private": true,
    "version": "0.0.0",
    ...
    "devDependencies": {
        "@launchmenu/core": "^0.0.5-alpha.13",
        "electron": "^9.3.1",
        "@launchmenu/applet-help": "^0.0.5-alpha.13",
        "@launchmenu/applet-applet-manager": "^0.0.5-alpha.13",
        "@launchmenu/applet-session-manager": "^0.0.5-alpha.13",
        "@launchmenu/applet-settings-manager": "^0.0.5-alpha.13",
        "@launchmenu/applet-window-manager": "^0.0.5-alpha.13"
    }
}
```
...
LaunchMenu applets are written in [TypeScript](https://www.typescriptlang.org/) (TS). It's also possible to make your applets in JavaScript, but we don't have any examples for javascript environment setup and we highly encourage the usage of TS.

In order to use TS, we will have to translate our TS source code to JavaScript before we're able to execute it. For this we can use the TS compiler. In order to simplify the setup, we provide our own [lm-build-tools](https://www.npmjs.com/package/@launchmenu/build-tools) module that can be used for this.
