...
This `Box` component also uses [Emotion's CSS prop object](https://emotion.sh/docs/css-prop#object-styles) to allow for inline CSS including selectors. Note that the `css` prop isn't supported for any react components other than `Box`.
