...
```ts title=IUILayer.ts link=https://github.com/LaunchMenu/LaunchMenu/blob/master/packages/core/src/uiLayers/_types/IUILayer.ts spoiler
...
Menus are used for - you guessed it - storing menu items. There are several types of standard menus, each implementing the `IMenu` interface:

```ts title=IMenu.ts link=https://github.com/LaunchMenu/LaunchMenu/blob/master/packages/core/src/menus/menu/_types/IMenu.ts spoiler
export type IMenu = {
    /**
